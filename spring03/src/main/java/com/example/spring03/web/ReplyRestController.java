...
// -> @RestController 클래스의 메서드 리턴값 => 클라이언트로 전송되는 값!
...
public class ReplyRestController {
    ...
    public ResponseEntity<List<ReplyReadDto>> readAllReplies(@PathVariable Integer postId){
        ...
        log.info("readAllReplies = {}" , postId);
        List<ReplyReadDto> list = replyService.readReplies(postId);
        log.info("# of list = {}", list.size());
        
        return ResponseEntity.ok(list);
        // Java의 List 타입 객체가 JSON(JavaScript Object Notation) 문자열로 변환되서
        // 클라이언트(브라우저)로 전송.
    }
    ...
    public ResponseEntity<Integer> updateReply(
        ...
        log.info("updateReply(reply={}, dto={})",replyId, dto);
        dto.setReplyId(replyId); // DTO에 댓글 아이디를 저장.
        Integer result = replyService.update(dto); // 수정된 댓글 번호 가져오기
