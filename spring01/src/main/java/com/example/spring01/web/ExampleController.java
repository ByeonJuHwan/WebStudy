...
public class ExampleController {
    ...
    // View Resolver(뷰룰 찾는 객체) 가 뷰(JSP)를 찾는 방법:
    ...
    public void example2(String userName, int age, Model model) {
        log.info("example2(userName={}, age={})", userName, age);
    ...
    @PostMapping("/ex3")
    public String example3(@RequestParam(name = "userName")String name, //(1)
    ...
    public String ex4(UserDto user, Model model) {
        log.info("ex4({})",user);
        // 1. DispatcherServlet이 요청 파라미터들을 분석하고 (reqeust.getParameter),
        // 2. UserDto의 기본 생성자를 호출해서 객체를 생성
        // 3. 요청 파라미터들의 이름으로 UserDto의 setter 메서드를 찾아서 호출.
        // 4. UserDto 객체를 컨트롤러 메서드를 호출할 때 argument롤 전달.
